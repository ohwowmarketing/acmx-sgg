...
function api_data_odds_nfl_current_season() {
  $url = 'https://fly.sportsdata.io/v3/nfl/scores/json/CurrentSeason';
...
function api_data_odds_nfl_current_week() {
  $url = 'https://fly.sportsdata.io/v3/nfl/scores/json/CurrentWeek';
  // $url = 'https://api.sportsdata.io/v3/nfl/scores/json/UpcomingWeek';
  $headers = api_headers_with_key( 'nfl' );
...
function api_data_odds_nfl( $selection ) {
  ...
  if ( $selected_week === '' ) {
    ...
    if ( $widget_schedule ) {
      $selected_week = api_data_odds_nfl_selected_week( $current_season, $widget_week );
    } else {
...
function api_data_odds( $league, $selection = '' ) {
  if ( $league === 'nfl' ) {
    ...
    $url = 'https://fly.sportsdata.io/v3/' . $league . '/odds/json/GameOddsByDate/' . $formatted_date;
